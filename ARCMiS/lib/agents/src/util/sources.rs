...
/// readable.
pub fn collect(root: &Path, per_file_cap: u64) -> anyhow::Result<Sources> {
...
impl Walk {
    ...
    fn dir(&self, dir: &Path, sources: &mut Sources) -> anyhow::Result<()> {
        let entries = read_dir(dir).with_context(|| format!("input root not readable at {}", dir.display()))?;
    ...
    fn file(&self, path: &Path, sources: &mut Sources) -> anyhow::Result<()> {
