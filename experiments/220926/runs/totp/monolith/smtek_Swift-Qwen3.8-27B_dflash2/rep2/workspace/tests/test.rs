use crate::{from_base32, hotp, hmac_sha1, sha1, pack32, unpack32, unpack64};

fn to_hex(a: &[u8], len: usize, buf: &mut String) {
    buf.clear();
    for i in 0..len {
...
fn test_sha1() {
    ...
    to_hex(&hash, 20, &mut str);
    assert_eq!(str, "da39a3ee5e6b4b0d3255bfef95601890afd80709");

    let abc = b"abc";
    buf[..abc.len()].copy_from_slice(abc);
    sha1(&mut buf, abc.len(), buf.len(), &mut hash);
    to_hex(&hash, 20, &mut str);
    assert_eq!(str, "a9993e364706816aba3e25717850c26c9cd0d89d");
    ...
    to_hex(&hash, 20, &mut str);
    assert_eq!(str, "2fd4e1c67a2d28fced849ee1bb76e7391b93eb12");
}
...
fn test_hmac_sha1() {
    ...
    to_hex(&hash, 20, &mut str);
    assert_eq!(str, "125d7342b9ac11cd91a39af48aa17b4f63f175d3");
}
