...
pub fn unpack64(x: u64, a: &mut [u8; 8]) {
    unpack32((x >> 32) as u32, &mut a[0..4]);
    unpack32(x as u32, &mut a[4..8]);
}
...
pub fn sha1(buf: &mut [u8], len: usize, cap: usize, hash: &mut [u8; 20]) -> i32 {
    ...
    for i in 0..new_len / 64 {
        let mut w = [0u32; 80];
        for t in 0..16 {
            let chunk: [u8; 4] = buf[i * 64 + t * 4..i * 64 + t * 4 + 4].try_into().unwrap();
            w[t] = pack32(&chunk);
        }
        for t in 16..80 {
    ...
    for i in 0..5 {
        unpack32(h[i], &mut hash[i * 4..i * 4 + 4]);
    }
...
pub fn hotp(key: &[u8; 64], counter: u64) -> i32 {
    ...
    let trunc = pack32(&hash[(hash[19] & 0xF) as usize..(hash[19] & 0xF) as usize + 4]) & 0x7FFFFFFF;
