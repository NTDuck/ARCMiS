//! TOTP / HOTP / SHA1 / HMAC-SHA1 / base32, translated from the C
//! implementation by Sijmen Mulder (see LICENSE.md).
...
pub fn sha1(buf: &mut [u8], len: usize, cap: usize, hash: &mut [u8; 20]) -> i32 {
    ...
    buf[len] = 1 << 7;
    unpack64((len as u64) * 8, &mut buf[new_len - 8..new_len].try_into().unwrap());
    ...
    for i in 0..new_len / 64 {
        let mut w = [0u32; 80];
        for t in 0..16 {
            let a = &buf[i * 64 + t * 4..i * 64 + t * 4 + 4];
            w[t] = pack32(a.try_into().unwrap());
        }
        for t in 16..80 {
        ...
        for t in 0..80 {
            ...
            let T = a.rotate_left(5).wrapping_add(f).wrapping_add(e).wrapping_add(K[t / 20]).wrapping_add(w[t]);
            e = d;
            d = c;
            c = b.rotate_left(30);
            b = a;
...
pub fn hotp(key: &[u8; 64], counter: u64) -> i32 {
    ...
    let trunc = pack32(&hash[(hash[19] & 0xF) as usize..(hash[19] & 0xF) as usize + 4].try_into().unwrap()) & 0x7FFFFFFF;
