//! Unit tests, translated from test.c

use totp::*;

fn to_hex(a: &[u8], buf: &mut [u8]) {
    for (i, &byte) in a.iter().enumerate() {
        buf[i * 2] = b"0123456789abcdef"[byte >> 4];
        buf[i * 2 + 1] = b"0123456789abcdef"[byte & 0xF];
    }
}
...
fn test_sha1() {
    ...
    to_hex(&hash, &mut str);
    assert_eq!(
        String::from_utf8(str[..40].to_vec()).unwrap(),
        "da39a3ee5e6b4b0d3255bfef95601890afd80709"
    );

    let s = "abc";
    buf[..s.len()].copy_from_slice(s.as_bytes());
    sha1(&mut buf, s.len(), buf.len(), &mut hash);
    to_hex(&hash, &mut str);
    assert_eq!(
        String::from_utf8(str[..40].to_vec()).unwrap(),
