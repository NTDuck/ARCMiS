...
use totp::{
    from_base32, hotp, hmac_sha1, pack32, sha1, unpack32, unpack64,
};
...
fn test_pack() {
    ...
    let mut chunk = [0u8; 4];
    chunk.copy_from_slice(&a[0..4]);
    assert_eq!(pack32(&chunk), 0x12345678);
}
...
fn test_sha1() {
    ...
    sha1(&mut buf, data.len(), buf.len(), &mut hash);
    assert_eq!(
        to_hex(&hash, 20),
