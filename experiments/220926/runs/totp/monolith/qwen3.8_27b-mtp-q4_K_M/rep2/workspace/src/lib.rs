...
//! 'Authenticator' apps.
//!
//! See RFC 4226 (HOTP), RFC 6238 (TOTP), RFC 2104 (HMAC), FIPS 180-3
//! (SHA1) and RFC 4648 (Base32).
...
pub fn sha1(buf: &mut [u8], len: usize, cap: usize, hash: &mut [u8; 20]) -> i32 {
    ...
    for i in 0..new_len / 64 {
        ...
        for t in 0..80 {
            ...
            let t_ = rotl(a, 5).wrapping_add(f).wrapping_add(e).wrapping_add(K[t / 20]).wrapping_add(w[t]);
            e = d;
            d = c;
            c = rotl(b, 30);
            b = a;
...
pub fn hotp(key: &[u8; 64], counter: u64) -> i32 {
    ...
    let chunk: [u8; 4] = hash[offset as usize..offset as usize + 4]
        .try_into()
        .unwrap();
